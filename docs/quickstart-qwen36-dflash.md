...
**`spec-type dflash is set but draft model is not a dflash model`.** The file passed to `--spec-draft-model` is not a DFlash draft GGUF. Make sure you are linking the DFlash draft model, not a regular Qwen model.
